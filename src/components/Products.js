           ...
           {props.products !== undefined ?
            ...
            <section className="flex flex-col flex-wrap sm:flex-col md:flex-col lg:flex-row mx-auto  justify-center p-2 ">
              ...
              {props?.products?.map((product, i) => {
                ...
                <article 
                  key={"article" + i}
                  className ="group lg:my-[250px] md:my-[100px] lg:w-1/3 lg:flex-wrap bg-blend-hard-light object-left  rounded-lg hover:drop-shadow-md">

                    {/* Image */}
                    <img  key={"img" + i} 
                          src={product.image}  
                          alt="" 
                          className="h-1/4 w-full sm:h-full sm:w-full object-scale-down mx-auto "
                    />

                    {/* Model title*/}
                    <div  key={"model" + i} 
                          className= "text-2xl text-center mt-2 p-2 text-red-900 font-extralight hover:text-gray-500 "> 
                          <a href={"/productos/" + product.id}> {renderSwitch(product.category)} - {product.model} </a> 
                    </div>
                    ...
                    <div  className = " relative bottom-16-z-50 bottom-[40px] flex  group-hover:z-10 group-hover:bottom-0  duration-500 flex-row mx-auto justify-between p-4 w-1/3 opacity-0 group-hover:opacity-100" >
                      <button class="bg-transparent hover:bg-red-900  text-red-900  font-semibold hover:text-white mr-3 py-2 px-4 border  border-red-900 hover:border-transparent rounded">
                          <a href ={"/productos/" + product.id}> Detalle</a>
                      </button>
                      <button class="bg-transparent hover:bg-red-900  text-red-900 font-semibold hover:text-white  py-2 px-4 border border-red-900 hover:border-transparent rounded">
                          <a href ="/"> Comprá</a>
                      </button>
                    </div>
